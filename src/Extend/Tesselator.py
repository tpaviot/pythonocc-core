...
from OCC.Core.BRep import (
    ...
    BRep_Tool_Polygon3D,
    BRep_Tool_PolygonOnTriangulation,
)
...
class EdgeDiscretizer:
    def __init__(self, a_curve):
        ...
        self._deviation = 0.3
        self._angular_deflection = 5.0 * pi / 180.0
    ...
    def compute_bounding_box(self):
        ...
        BndLib_Add3dCurve_Add(
            self._curve_adaptator, self._U1, self._U2, 0.0, curve_bounding_box
        )

        x_min, y_min, z_min, x_max, y_max, z_max = curve_bounding_box.Get()

        dx, dy, dz = x_max - x_min, y_max - y_min, z_max - z_min
        px, py, pz = x_max + dx / 2.0, y_max + dy / 2.0, z_min + dz / 2.0
        ...
        # other cases
            ...
            for j in range(1, nb_inter + 1):
                ...
                if current_u1 < self._U2 and current_u2 > self._U1:
                    ...
                    algo = GCPnts_TangentialDeflection(
                        self._curve_adaptator,
                        self._U1,
                        self._U2,
...
class Tesselator:
    ...
    def set_current_lod_level(self, ratio):
        ...
        if not ratio in self._lod:
            ...
            self._vertex_coords, self._vertex_indices, self._normal_coords = self._lod[
                ratio
            ]
    ...
    def compute_bounding_box(self):
        ...
        self._bs_radius = max(self._bb_size) / 2.0
    ...
    def tesselate(self):
        ...
        for nt in range(myT.NbTriangles()):
            ...
            if orient not in [
                ...
                TopAbs_Orientation.TopAbs_INTERNAL,
            ]:
...
class ShapeTesselator(Tesselator):
    ...
    def tesselate(self):
        ...
        # Triangulate
        msh_algo = BRepMesh_DiscretFactory_Get().Discret(
            self._shape,
        ...
        while face_explorer.More():
            ...
            indices_translated = [
                [a - 1 + advance for a in l] for l in face_vertex_indices
            ]
        ...
        self._lod[0.0] = (all_vertex_coords, all_vertex_indices, all_normal_coords)
...
if __name__ == "__main__":
    ...
    init_time = time.perf_counter()
    tess = ShapeTesselator(
        bo_t, compute_normals=True, compute_edges=True, mesh_quality=0.1
    )
    ...
    helix_edge = BRepBuilderAPI_MakeEdge(
        aSegment.Value(), aCylinder, 0.0, 6.0 * pi
    ).Edge()
    ...
    torus = BRepPrimAPI_MakeTorus(30.0, 10).Shape()
    box = BRepPrimAPI_MakeBox(10, 20, 30).Shape()
    # loads brep shape
