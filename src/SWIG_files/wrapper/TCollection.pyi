from enum import IntEnum
import typing
from typing import Any, overload, NewType, Optional, Tuple

from OCC.Core.Standard import *
from OCC.Core.NCollection import *


class tcollection:
    @staticmethod
    def NextPrimeForMap(I: int) -> int: ...

class TCollection_AsciiString:
    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, theStringView: Any) -> None: ...
    @overload
    def __init__(self, theMessage: str) -> None: ...
    @overload
    def __init__(self, theMessage: str, theLength: int) -> None: ...
    @overload
    def __init__(self, theChar: Any) -> None: ...
    @overload
    def __init__(self, theLength: int, theFiller: Any) -> None: ...
    @overload
    def __init__(self, theValue: int) -> None: ...
    @overload
    def __init__(self, theValue: float) -> None: ...
    @overload
    def __init__(self, theString: str) -> None: ...
    @overload
    def __init__(self, theOther: str) -> None: ...
    @overload
    def __init__(self, theString: str, theChar: Any) -> None: ...
    @overload
    def __init__(self, theString: str, theMessage: str) -> None: ...
    @overload
    def __init__(self, theString: str, theOtherString: str) -> None: ...
    @overload
    def __init__(self, theExtendedString: str, theReplaceNonAscii: Optional[Any] = 0) -> None: ...
    @overload
    def __init__(self, theStringUtf: Any) -> None: ...
    @overload
    def AssignCat(self, theOther: Any) -> None: ...
    @overload
    def AssignCat(self, theOther: int) -> None: ...
    @overload
    def AssignCat(self, theOther: float) -> None: ...
    @overload
    def AssignCat(self, theOther: str, theReplaceNonAscii: Optional[Any] = 0) -> None: ...
    @overload
    def AssignCat(self, theStringUtf: Any) -> None: ...
    @overload
    def AssignCat(self, theString: str, theLength: int) -> None: ...
    @overload
    def AssignCat(self, theOther: str) -> None: ...
    @overload
    def AssignCat(self, theCString: str) -> None: ...
    @overload
    def AssignCat(self, theStringView: Any) -> None: ...
    def Capitalize(self) -> None: ...
    @overload
    def Cat(self, theString: str, theLength: int) -> str: ...
    @overload
    def Cat(self, theOther: Any) -> str: ...
    @overload
    def Cat(self, theOther: int) -> str: ...
    @overload
    def Cat(self, theOther: float) -> str: ...
    @overload
    def Cat(self, theOther: str, theReplaceNonAscii: Optional[Any] = 0) -> str: ...
    @overload
    def Cat(self, theStringUtf: Any) -> str: ...
    @overload
    def Cat(self, theOther: str) -> str: ...
    @overload
    def Cat(self, theCString: str) -> str: ...
    @overload
    def Cat(self, theStringView: Any) -> str: ...
    def Center(self, theWidth: int, theFiller: Any) -> None: ...
    def ChangeAll(self, theChar: Any, theNewChar: Any, theCaseSensitive: Optional[bool] = True) -> None: ...
    def Clear(self) -> None: ...
    @overload
    def Copy(self, theString: str, theLength: int) -> None: ...
    @overload
    def Copy(self, theCString: str) -> None: ...
    @overload
    def Copy(self, theStringView: Any) -> None: ...
    @overload
    def Copy(self, theFromWhere: str) -> None: ...
    @staticmethod
    def EmptyString() -> str: ...
    @overload
    def EndsWith(self, theEndString: str, theEndLength: int) -> bool: ...
    @overload
    def EndsWith(self, theEndString: str) -> bool: ...
    @overload
    def EndsWith(self, theEndString: Any) -> bool: ...
    @overload
    def FirstLocationInSet(self, theSet: str, theSetLength: int, theFromIndex: int, theToIndex: int) -> int: ...
    @overload
    def FirstLocationInSet(self, theSet: str, theFromIndex: int, theToIndex: int) -> int: ...
    @overload
    def FirstLocationInSet(self, theSet: Any, theFromIndex: int, theToIndex: int) -> int: ...
    @overload
    def FirstLocationNotInSet(self, theSet: str, theSetLength: int, theFromIndex: int, theToIndex: int) -> int: ...
    @overload
    def FirstLocationNotInSet(self, theSet: str, theFromIndex: int, theToIndex: int) -> int: ...
    @overload
    def FirstLocationNotInSet(self, theSet: Any, theFromIndex: int, theToIndex: int) -> int: ...
    def HashCode(self) -> int: ...
    @overload
    def Insert(self, theWhere: int, theWhat: Any) -> None: ...
    @overload
    def Insert(self, theWhere: int, theString: str, theLength: int) -> None: ...
    @overload
    def Insert(self, theWhere: int, theWhat: str) -> None: ...
    @overload
    def Insert(self, theWhere: int, theCString: str) -> None: ...
    @overload
    def Insert(self, theWhere: int, theStringView: Any) -> None: ...
    @overload
    def InsertAfter(self, theIndex: int, theString: str, theLength: int) -> None: ...
    @overload
    def InsertAfter(self, theIndex: int, theOther: str) -> None: ...
    @overload
    def InsertAfter(self, theIndex: int, theCString: str) -> None: ...
    @overload
    def InsertAfter(self, theIndex: int, theStringView: Any) -> None: ...
    @overload
    def InsertBefore(self, theIndex: int, theString: str, theLength: int) -> None: ...
    @overload
    def InsertBefore(self, theIndex: int, theOther: str) -> None: ...
    @overload
    def InsertBefore(self, theIndex: int, theCString: str) -> None: ...
    @overload
    def InsertBefore(self, theIndex: int, theStringView: Any) -> None: ...
    def IntegerValue(self) -> int: ...
    def IsAscii(self) -> bool: ...
    @overload
    def IsDifferent(self, theOther: str) -> bool: ...
    @overload
    def IsDifferent(self, theString: str, theLength: int) -> bool: ...
    @overload
    def IsDifferent(self, theCString: str) -> bool: ...
    @overload
    def IsDifferent(self, theStringView: Any) -> bool: ...
    def IsEmpty(self) -> bool: ...
    @overload
    def IsEqual(self, theOther: str) -> bool: ...
    @overload
    def IsEqual(self, theString: str, theLength: int) -> bool: ...
    @overload
    def IsEqual(self, theCString: str) -> bool: ...
    @overload
    def IsEqual(self, theStringView: Any) -> bool: ...
    @overload
    @staticmethod
    def IsEqual(string1: str, string2: str) -> bool: ...
    @overload
    @staticmethod
    def IsEqual(theString1: str, theStringView: Any) -> bool: ...
    @overload
    @staticmethod
    def IsEqual(theStringView: Any, theString2: str) -> bool: ...
    @overload
    def IsGreater(self, theString: str, theLength: int) -> bool: ...
    @overload
    def IsGreater(self, theOther: str) -> bool: ...
    @overload
    def IsGreater(self, theCString: str) -> bool: ...
    @overload
    def IsGreater(self, theStringView: Any) -> bool: ...
    def IsIntegerValue(self) -> bool: ...
    @overload
    def IsLess(self, theString: str, theLength: int) -> bool: ...
    @overload
    def IsLess(self, theOther: str) -> bool: ...
    @overload
    def IsLess(self, theCString: str) -> bool: ...
    @overload
    def IsLess(self, theStringView: Any) -> bool: ...
    def IsRealValue(self, theToCheckFull: Optional[bool] = False) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theString1: str, theLength1: int, theString2: str, theLength2: int, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theString1: str, theString2: str, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theString1: str, theCString: str, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theCString: str, theString2: str, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theString1: str, theStringView: Any, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theStringView: Any, theString2: str, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theCString1: str, theCString2: str, theIsCaseSensitive: bool) -> bool: ...
    @overload
    @staticmethod
    def IsSameString(theStringView1: Any, theStringView2: Any, theIsCaseSensitive: bool) -> bool: ...
    def LeftAdjust(self) -> None: ...
    def LeftJustify(self, theWidth: int, theFiller: Any) -> None: ...
    def Length(self) -> int: ...
    @overload
    def Location(self, theOther: str, theFromIndex: int, theToIndex: int) -> int: ...
    @overload
    def Location(self, theN: int, theC: Any, theFromIndex: int, theToIndex: int) -> int: ...
    def LowerCase(self) -> None: ...
    def Prepend(self, theOther: str) -> None: ...
    def Print(self) -> str: ...
    def Read(self, theStream: str) -> None: ...
    def RealValue(self) -> float: ...
    def Remove(self, theWhere: int, theHowMany: Optional[int] = 1) -> None: ...
    @overload
    def RemoveAll(self, theC: Any, theCaseSensitive: bool) -> None: ...
    @overload
    def RemoveAll(self, theWhat: Any) -> None: ...
    def RightAdjust(self) -> None: ...
    def RightJustify(self, theWidth: int, theFiller: Any) -> None: ...
    @overload
    def Search(self, theWhat: str, theWhatLength: int) -> int: ...
    @overload
    def Search(self, theWhat: str) -> int: ...
    @overload
    def Search(self, theCString: str) -> int: ...
    @overload
    def Search(self, theWhat: Any) -> int: ...
    @overload
    def SearchFromEnd(self, theWhat: str, theWhatLength: int) -> int: ...
    @overload
    def SearchFromEnd(self, theWhat: str) -> int: ...
    @overload
    def SearchFromEnd(self, theCString: str) -> int: ...
    @overload
    def SearchFromEnd(self, theWhat: Any) -> int: ...
    @overload
    def SetValue(self, theWhere: int, theWhat: Any) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theString: str, theLength: int) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theWhat: str) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theCString: str) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theStringView: Any) -> None: ...
    def Split(self, theWhere: int) -> str: ...
    @overload
    def StartsWith(self, theStartString: str, theStartLength: int) -> bool: ...
    @overload
    def StartsWith(self, theStartString: str) -> bool: ...
    @overload
    def StartsWith(self, theCString: str) -> bool: ...
    @overload
    def StartsWith(self, theStartString: Any) -> bool: ...
    def SubString(self, theFromIndex: int, theToIndex: int) -> str: ...
    def Swap(self, theOther: str) -> None: ...
    def ToCString(self) -> str: ...
    def Token(self, theSeparators: Optional[str] = "\t", theWhichOne: Optional[int] = 1) -> str: ...
    def Trunc(self, theHowMany: int) -> None: ...
    def UpperCase(self) -> None: ...
    def UsefullLength(self) -> int: ...
    def Value(self, theWhere: int) -> Any: ...
    def __str__(self) -> str: ...

class TCollection_ExtendedString:
    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, theString: str, theIsMultiByte: Optional[bool] = False) -> None: ...
    @overload
    def __init__(self, theString: Any) -> None: ...
    @overload
    def __init__(self, theStringUtf: Any) -> None: ...
    @overload
    def __init__(self, theChar: Any) -> None: ...
    @overload
    def __init__(self, theChar: Any) -> None: ...
    @overload
    def __init__(self, theLength: int, theFiller: Any) -> None: ...
    @overload
    def __init__(self, theValue: int) -> None: ...
    @overload
    def __init__(self, theValue: float) -> None: ...
    @overload
    def __init__(self, theString: str) -> None: ...
    @overload
    def __init__(self, theOther: str) -> None: ...
    @overload
    def __init__(self, theString: str, theIsMultiByte: Optional[bool] = True) -> None: ...
    @overload
    def __init__(self, theString: Any, theLength: int) -> None: ...
    @overload
    def AssignCat(self, theOther: str) -> None: ...
    @overload
    def AssignCat(self, theOther: int) -> None: ...
    @overload
    def AssignCat(self, theChar: Any) -> None: ...
    @overload
    def AssignCat(self, theOther: float) -> None: ...
    @overload
    def AssignCat(self, theChar: Any) -> None: ...
    @overload
    def AssignCat(self, theString: Any, theLength: int) -> None: ...
    @overload
    def AssignCat(self, theString: Any) -> None: ...
    def Capitalize(self) -> None: ...
    @overload
    def Cat(self, theOther: Any, theLength: int) -> str: ...
    @overload
    def Cat(self, theOther: Any) -> str: ...
    @overload
    def Cat(self, theOther: int) -> str: ...
    @overload
    def Cat(self, theOther: float) -> str: ...
    @overload
    def Cat(self, theChar: Any) -> str: ...
    @overload
    def Cat(self, theChar: Any) -> str: ...
    @overload
    def Cat(self, theOther: str) -> str: ...
    def Center(self, theWidth: int, theFiller: Any) -> None: ...
    def ChangeAll(self, theChar: Any, theNewChar: Any) -> None: ...
    def Clear(self) -> None: ...
    @overload
    def Copy(self, theString: Any, theLength: int) -> None: ...
    @overload
    def Copy(self, theString: Any) -> None: ...
    @overload
    def Copy(self, theFromWhere: str) -> None: ...
    @staticmethod
    def EmptyString() -> str: ...
    @overload
    def EndsWith(self, theEndString: Any, theLength: int) -> bool: ...
    @overload
    def EndsWith(self, theEndString: Any) -> bool: ...
    @overload
    def EndsWith(self, theEndString: str) -> bool: ...
    def FirstLocationInSet(self, theSet: str, theFromIndex: int, theToIndex: int) -> int: ...
    def FirstLocationNotInSet(self, theSet: str, theFromIndex: int, theToIndex: int) -> int: ...
    def HashCode(self) -> int: ...
    @overload
    def Insert(self, theWhere: int, theWhat: Any) -> None: ...
    @overload
    def Insert(self, theWhere: int, theWhat: Any, theLength: int) -> None: ...
    @overload
    def Insert(self, theWhere: int, theWhat: Any) -> None: ...
    @overload
    def Insert(self, theWhere: int, theWhat: str) -> None: ...
    def IntegerValue(self) -> int: ...
    def IsAscii(self) -> bool: ...
    @overload
    def IsDifferent(self, theOther: Any, theLength: int) -> bool: ...
    @overload
    def IsDifferent(self, theOther: Any) -> bool: ...
    @overload
    def IsDifferent(self, theOther: str) -> bool: ...
    def IsEmpty(self) -> bool: ...
    @overload
    def IsEqual(self, theOther: Any, theLength: int) -> bool: ...
    @overload
    def IsEqual(self, theOther: Any) -> bool: ...
    @overload
    def IsEqual(self, theOther: str) -> bool: ...
    @overload
    @staticmethod
    def IsEqual(theString1: str, theString2: str) -> bool: ...
    @overload
    def IsGreater(self, theOther: Any, theLength: int) -> bool: ...
    @overload
    def IsGreater(self, theOther: Any) -> bool: ...
    @overload
    def IsGreater(self, theOther: str) -> bool: ...
    def IsIntegerValue(self) -> bool: ...
    @overload
    def IsLess(self, theOther: Any, theLength: int) -> bool: ...
    @overload
    def IsLess(self, theOther: Any) -> bool: ...
    @overload
    def IsLess(self, theOther: str) -> bool: ...
    def IsRealValue(self, theToCheckFull: Optional[bool] = False) -> bool: ...
    def IsSameString(self, theOther: str, theIsCaseSensitive: bool) -> bool: ...
    def LeftAdjust(self) -> None: ...
    def LeftJustify(self, theWidth: int, theFiller: Any) -> None: ...
    def Length(self) -> int: ...
    def LengthOfCString(self) -> int: ...
    @overload
    def Prepend(self, theOther: Any, theLength: int) -> None: ...
    @overload
    def Prepend(self, theOther: Any) -> None: ...
    @overload
    def Prepend(self, theOther: str) -> None: ...
    def Print(self) -> str: ...
    def RealValue(self) -> float: ...
    def Remove(self, theWhere: int, theHowMany: Optional[int] = 1) -> None: ...
    def RemoveAll(self, theWhat: Any) -> None: ...
    def RightAdjust(self) -> None: ...
    def RightJustify(self, theWidth: int, theFiller: Any) -> None: ...
    @overload
    def Search(self, theWhat: Any, theLength: int) -> int: ...
    @overload
    def Search(self, theWhat: Any) -> int: ...
    @overload
    def Search(self, theWhat: str) -> int: ...
    @overload
    def SearchFromEnd(self, theWhat: Any, theLength: int) -> int: ...
    @overload
    def SearchFromEnd(self, theWhat: Any) -> int: ...
    @overload
    def SearchFromEnd(self, theWhat: str) -> int: ...
    @overload
    def SetValue(self, theWhere: int, theWhat: Any) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theWhat: Any, theLength: int) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theWhat: Any) -> None: ...
    @overload
    def SetValue(self, theWhere: int, theWhat: str) -> None: ...
    def Split(self, theWhere: int) -> str: ...
    @overload
    def StartsWith(self, theStartString: Any, theLength: int) -> bool: ...
    @overload
    def StartsWith(self, theStartString: Any) -> bool: ...
    @overload
    def StartsWith(self, theStartString: str) -> bool: ...
    def SubString(self, theFromIndex: int, theToIndex: int) -> str: ...
    def Swap(self, theOther: str) -> None: ...
    def ToExtString(self) -> Any: ...
    def ToUTF8CString(self, theCString: Standard_PCharacter) -> int: ...
    def Token(self, theSeparators: Any, theWhichOne: Optional[int] = 1) -> str: ...
    def Trunc(self, theHowMany: int) -> None: ...
    def Value(self, theWhere: int) -> Any: ...
    def __str__(self) -> str: ...

class TCollection_HAsciiString(Standard_Transient):
    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, message: str) -> None: ...
    @overload
    def __init__(self, aChar: Any) -> None: ...
    @overload
    def __init__(self, length: int, filler: Any) -> None: ...
    @overload
    def __init__(self, value: int) -> None: ...
    @overload
    def __init__(self, value: float) -> None: ...
    @overload
    def __init__(self, aString: str) -> None: ...
    @overload
    def __init__(self, theString: str) -> None: ...
    @overload
    def __init__(self, aString: TCollection_HAsciiString) -> None: ...
    @overload
    def __init__(self, aString: TCollection_HExtendedString, replaceNonAscii: Any) -> None: ...
    @overload
    def AssignCat(self, other: str) -> None: ...
    @overload
    def AssignCat(self, other: TCollection_HAsciiString) -> None: ...
    def Capitalize(self) -> None: ...
    @overload
    def Cat(self, other: str) -> TCollection_HAsciiString: ...
    @overload
    def Cat(self, other: TCollection_HAsciiString) -> TCollection_HAsciiString: ...
    def Center(self, Width: int, Filler: Any) -> None: ...
    def ChangeAll(self, aChar: Any, NewChar: Any, CaseSensitive: Optional[bool] = True) -> None: ...
    def Clear(self) -> None: ...
    def FirstLocationInSet(self, Set: TCollection_HAsciiString, FromIndex: int, ToIndex: int) -> int: ...
    def FirstLocationNotInSet(self, Set: TCollection_HAsciiString, FromIndex: int, ToIndex: int) -> int: ...
    @overload
    def Insert(self, where: int, what: Any) -> None: ...
    @overload
    def Insert(self, where: int, what: str) -> None: ...
    @overload
    def Insert(self, where: int, what: TCollection_HAsciiString) -> None: ...
    def InsertAfter(self, Index: int, other: TCollection_HAsciiString) -> None: ...
    def InsertBefore(self, Index: int, other: TCollection_HAsciiString) -> None: ...
    def IntegerValue(self) -> int: ...
    def IsAscii(self) -> bool: ...
    def IsDifferent(self, S: TCollection_HAsciiString) -> bool: ...
    def IsEmpty(self) -> bool: ...
    def IsGreater(self, other: TCollection_HAsciiString) -> bool: ...
    def IsIntegerValue(self) -> bool: ...
    def IsLess(self, other: TCollection_HAsciiString) -> bool: ...
    def IsRealValue(self) -> bool: ...
    def IsSameState(self, other: TCollection_HAsciiString) -> bool: ...
    @overload
    def IsSameString(self, S: TCollection_HAsciiString) -> bool: ...
    @overload
    def IsSameString(self, S: TCollection_HAsciiString, CaseSensitive: bool) -> bool: ...
    def LeftAdjust(self) -> None: ...
    def LeftJustify(self, Width: int, Filler: Any) -> None: ...
    def Length(self) -> int: ...
    @overload
    def Location(self, other: TCollection_HAsciiString, FromIndex: int, ToIndex: int) -> int: ...
    @overload
    def Location(self, N: int, C: Any, FromIndex: int, ToIndex: int) -> int: ...
    def LowerCase(self) -> None: ...
    def Prepend(self, other: TCollection_HAsciiString) -> None: ...
    def Print(self) -> str: ...
    def RealValue(self) -> float: ...
    def Remove(self, where: int, ahowmany: Optional[int] = 1) -> None: ...
    @overload
    def RemoveAll(self, C: Any, CaseSensitive: bool) -> None: ...
    @overload
    def RemoveAll(self, what: Any) -> None: ...
    def RightAdjust(self) -> None: ...
    def RightJustify(self, Width: int, Filler: Any) -> None: ...
    @overload
    def Search(self, what: str) -> int: ...
    @overload
    def Search(self, what: TCollection_HAsciiString) -> int: ...
    @overload
    def SearchFromEnd(self, what: str) -> int: ...
    @overload
    def SearchFromEnd(self, what: TCollection_HAsciiString) -> int: ...
    @overload
    def SetValue(self, where: int, what: Any) -> None: ...
    @overload
    def SetValue(self, where: int, what: str) -> None: ...
    @overload
    def SetValue(self, where: int, what: TCollection_HAsciiString) -> None: ...
    def Split(self, where: int) -> TCollection_HAsciiString: ...
    def String(self) -> str: ...
    def SubString(self, FromIndex: int, ToIndex: int) -> TCollection_HAsciiString: ...
    def ToCString(self) -> str: ...
    def Token(self, separators: Optional[str] = "\t", whichone: Optional[int] = 1) -> TCollection_HAsciiString: ...
    def Trunc(self, ahowmany: int) -> None: ...
    def UpperCase(self) -> None: ...
    def UsefullLength(self) -> int: ...
    def Value(self, where: int) -> Any: ...

class TCollection_HExtendedString(Standard_Transient):
    @overload
    def __init__(self) -> None: ...
    @overload
    def __init__(self, message: str) -> None: ...
    @overload
    def __init__(self, message: Any) -> None: ...
    @overload
    def __init__(self, aChar: Any) -> None: ...
    @overload
    def __init__(self, length: int, filler: Any) -> None: ...
    @overload
    def __init__(self, aString: str) -> None: ...
    @overload
    def __init__(self, theString: str) -> None: ...
    @overload
    def __init__(self, aString: TCollection_HAsciiString) -> None: ...
    @overload
    def __init__(self, aString: TCollection_HExtendedString) -> None: ...
    def AssignCat(self, other: TCollection_HExtendedString) -> None: ...
    def Cat(self, other: TCollection_HExtendedString) -> TCollection_HExtendedString: ...
    def ChangeAll(self, aChar: Any, NewChar: Any) -> None: ...
    def Clear(self) -> None: ...
    @overload
    def Insert(self, where: int, what: Any) -> None: ...
    @overload
    def Insert(self, where: int, what: TCollection_HExtendedString) -> None: ...
    def IsAscii(self) -> bool: ...
    def IsEmpty(self) -> bool: ...
    def IsGreater(self, other: TCollection_HExtendedString) -> bool: ...
    def IsLess(self, other: TCollection_HExtendedString) -> bool: ...
    def IsSameState(self, other: TCollection_HExtendedString) -> bool: ...
    def Length(self) -> int: ...
    def Print(self) -> str: ...
    def Remove(self, where: int, ahowmany: Optional[int] = 1) -> None: ...
    def RemoveAll(self, what: Any) -> None: ...
    def Search(self, what: TCollection_HExtendedString) -> int: ...
    def SearchFromEnd(self, what: TCollection_HExtendedString) -> int: ...
    @overload
    def SetValue(self, where: int, what: Any) -> None: ...
    @overload
    def SetValue(self, where: int, what: TCollection_HExtendedString) -> None: ...
    def Split(self, where: int) -> TCollection_HExtendedString: ...
    def String(self) -> str: ...
    def ToExtString(self) -> Any: ...
    def Token(self, separators: Any, whichone: Optional[int] = 1) -> TCollection_HExtendedString: ...
    def Trunc(self, ahowmany: int) -> None: ...
    def Value(self, where: int) -> Any: ...

# harray1 classes
# harray2 classes
# hsequence classes

