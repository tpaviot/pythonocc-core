...
(although the function IsNormal does exist).") Angular;
...
space which contains some small objects of the complete scene.") Confusion;
...
Created for speed and convenience.") SquareConfusion;
...
algorithm, but this option might be costly.") Approximation;
...
tolerance to your own algorithm.") PConfusion;
...
-   Precision::Intersection() / 100., or Precision::Intersection() / T.") PIntersection;
...
segment whose length is equal to 100. (default value), or T.  
The parametric tolerance of intersection is equal to :  
-   Precision::Approximation() / 100., or Precision::Approximation() / T.") PApproximation;
...
Convert a real  space precision  to  a  parametric  
         ...
         Value is Parametric(P,1.e+2)") Parametric;
...
Used  to test distances  in parametric  space on a  
         ...
         This is Precision::Parametric(Precision::Confusion())") PConfusion;
...
Used for Intersections  in parametric  space  on a  
         ...
         This is Precision::Parametric(Precision::Intersection())") PIntersection;
...
Used for  Approximations  in parametric space on a  
         ...
         This is Precision::Parametric(Precision::Approximation())") PApproximation;
...
Returns True if R may be considered as an infinite  
         number. Currently Abs(R) > 1e100") IsInfinite;
...
Returns True if R may be considered as  a positive  
         infinite number. Currently R > 1e100") IsPositiveInfinite;
...
Returns True if R may  be considered as a negative  
         infinite number. Currently R < -1e100") IsNegativeInfinite;
...
Returns a  big number that  can  be  considered as  
         infinite. Use -Infinite() for a negative big number.") Infinite;
