import os

from OCC.Graphic3d import Graphic3d_MaterialAspect, Graphic3d_NOM_PLASTIC
from OCC.MeshVS import (MeshVS_Mesh, MeshVS_MeshPrsBuilder,
                        MeshVS_DA_InteriorColor, MeshVS_DA_FrontMaterial,
                        MeshVS_DA_DisplayNodes,
                        MeshVS_DA_ShowEdges, MeshVS_DA_SmoothShading,
                        MeshVS_DA_ColorReflection,
                        MeshVS_DA_Reflection)
from OCC.Quantity import Quantity_Color, Quantity_TOC_RGB

try:
    from OCC.SMESH import SMESH_MeshVSLink, SMESH_Gen
except ImportError:
    print "this example requires SMESH to be installed... this module could " \
          "not be imported..."


def load_stl_fast(_path):
    """ loads an STL file

    the more well know OCC.StlAPI module is rather silly; it creates BRep
    faces from all the mesh faces. this results in very, very slow importing
    of STL files.

    9/10 times, an STL file will be utilized for display, rather
    than reconstructing BRep data from the STL file.

    For these cases, this function will "just" render the STL file in the
    viewer... and does so swiftly for large meshes

    Parameters
    ----------

    _path: str
        path to an STL file

    Returns
    -------
    OCC.SMESH.SMESH_Mesh

    Raises
    ------
    ValueError
        when an invalid path is found or a mesh that has no vertices

    TypeError
        no support for paths in unicode

    """
    gen = SMESH_Gen()
    mesh = gen.CreateMesh(0, True)

    _path = str(_path) if not isinstance(_path, str) else _path

    if not os.path.isfile(_path):
        raise ValueError("{0} is not a valid path".format(_path))

    if isinstance(_path, unicode):
        raise TypeError("only paths in str type supported, got a path in "
                        "unicode")

    mesh.STLToMesh(_path)
    # test for a minimal number of mesh vertices
    if mesh.NbNodes() < 1:
        raise ValueError("stl file {0} contains no geometry".format(_path))
    return mesh


def meshvs_link_from_mesh(mesh):
    """
    create a renderable instance from a ``mesh``
    the returned MeshVS_Mesh is a subclass of AIS_InteractiveShape

    Parameters
    ----------
    mesh : OCC.SMESH.SMESH_Mesh

    Returns
    -------
    MeshVS_Mesh

    """
    aDS = SMESH_MeshVSLink(mesh)
    aMeshVS = MeshVS_Mesh(True)
    DMF = 2  # to wrap; 1 is wireframe
    aPrsBuilder = MeshVS_MeshPrsBuilder(aMeshVS.GetHandle(), DMF,
                                        aDS.GetHandle())

    aMeshVS.SetDataSource(aDS.GetHandle())
    aMeshVS.AddBuilder(aPrsBuilder.GetHandle(), False)
    MeshVS_DMF_Shading = int(0x0002)  # 1 -> wireframe, 2 -> shaded
    aMeshVS.SetDisplayMode(MeshVS_DMF_Shading)
    # Create the graphic window and display the mesh
    mesh_to_display = aMeshVS.GetHandle()
    # display the mesh edges in black
    mesh_drawer = aMeshVS.GetDrawer().GetObject()
    mesh_drawer.SetColor(MeshVS_DA_InteriorColor,
                         Quantity_Color(1, 0, 0, Quantity_TOC_RGB))
    mesh_drawer.SetMaterial(MeshVS_DA_FrontMaterial,
                            Graphic3d_MaterialAspect(Graphic3d_NOM_PLASTIC))
    mesh_drawer.SetBoolean(MeshVS_DA_DisplayNodes, False)
    mesh_drawer.SetBoolean(MeshVS_DA_ShowEdges, False)
    mesh_drawer.SetBoolean(MeshVS_DA_SmoothShading, True)
    mesh_drawer.SetBoolean(MeshVS_DA_ColorReflection, True)
    mesh_drawer.SetBoolean(MeshVS_DA_Reflection, True)
    msh = mesh_to_display.GetObject()
    return msh


def get_fast_stl_mesh_from_path(_path):
    """
    open/parse STL file and get the resulting TopoDS_Shape instance

    Returns
    -------
    MeshVS_Mesh

    """
    mesh = load_stl_fast(_path)
    msh = meshvs_link_from_mesh(mesh)
    return msh


if __name__ == "__main__":
    from OCC.Display.SimpleGui import init_display

    display, start_display, add_menu, add_function_to_menu = init_display()
    directory = os.path.split(__file__)[0]
    pth = os.path.abspath(os.path.join(directory, "models", "fan.stl"))
    mesh = get_fast_stl_mesh_from_path(pth)
    display.Context.Display(mesh.GetHandle())
    start_display()
