...
def get_boundingbox(shape, tol=1e-6, as_vec=False):
    ...
    as_vec : bool
        wether to return the lower and upper point of the bounding box as
        gp_Vec instances
    ...
    -------
        ...
        if `as_vec` is False, return a tuple of lower and then upper X,Y,Z
        values representing the bounding box
    """
    bbox = Bnd_Box()
    bbox.SetGap(tol)
